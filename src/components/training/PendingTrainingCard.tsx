...
export function PendingTrainingCard({ completion }: { completion: any }) {
  ...
  return (
    <div className={`card p-4 flex items-center gap-4 ${isPast ? 'border-amber-200 dark:border-amber-800' : ''}`}>
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isPast ? 'bg-amber-50 dark:bg-amber-900/20' : 'bg-brand-50 dark:bg-brand-900/20'}`}>
      ...
      <div className="flex gap-2 shrink-0">
        <a
          href={`/training/new?scheduled=${completion.id}&modality=${training.modality ?? ''}&distance=${training.distance_meters ?? ''}&objective=${encodeURIComponent(training.objective ?? '')}`}
